...
- YouTube видео → `/vo` озвучка на русском через vot-cli
...
### Команды бота:
- `/vo <url>` — озвучка YouTube видео на русском (vot-cli)
...
### Озвучка YouTube видео (vot-cli)

Команда `/vo` позволяет получить озвучку YouTube видео на русском языке.

**Как работает (умный выбор метода, приоритет):**

```
1. YouTube Dubbed Track  → официальный AI-дубляж от Google (если есть) 🎬
2. vot-cli              → Yandex Voice-Over для видео < 4 часов 🎙
3. Субтитры + TTS       → fallback для длинных видео 📝
```

1. Отправляешь `/vo https://youtube.com/watch?v=xxx`
2. Бот ищет русскую аудиодорожку на YouTube (Google AI Dubbed)
3. Если есть — скачивает её напрямую (быстро, качественно)
4. Если нет и видео < 4ч — использует vot-cli (Yandex)
5. Если нет и видео > 4ч — субтитры → перевод → Edge TTS
6. Сохраняет mp3 и добавляет в RSS ленту

**Что это такое:**
- **YouTube Dubbed** (приоритет 1): Google выпустил AI-дубляж для YouTube. Если видео имеет официальную русскую аудиодорожку — скачиваем её напрямую через yt-dlp. Лучшее качество, работает для любой длительности.
- **vot-cli** (приоритет 2): Yandex Voice-Over Translation API. Работает для большинства английских видео < 4 часов.
- **Субтитры + TTS** (приоритет 3): Fallback для длинных видео без дубляжа. Скачиваем субтитры, переводим, озвучиваем через Edge TTS.
...
**Реализация:**
- `app/proc/voiceover.go` — VoiceoverService (vot-cli + YouTube Dubbed)
  - `GetDubbedAudioTracks()` — получить список аудиодорожек через yt-dlp --dump-json
  - `FindDubbedTrack()` — найти русскую дорожку
  - `DownloadDubbedTrack()` — скачать конкретную дорожку
  - `TranslateVideo()` — vot-cli fallback
