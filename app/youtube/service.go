...
type FeedInfo struct {
	...
	Author      string      `yaml:"author"` // podcast-level author; empty falls back to first entry's channel
}
...
func (s *Service) RSSFeed(fi FeedInfo) (string, error) {
	...
	for _, entry := range entries {
		...
		if entry.Duration > 0 {
			...
			m := (entry.Duration % 3600) / 60
			sec := entry.Duration % 60
			if h > 0 {
				duration = fmt.Sprintf("%d:%02d:%02d", h, m, sec)
			} else {
				duration = fmt.Sprintf("%d:%02d", m, sec)
			}
		}

		var itunesImage *rssfeed.ItunesImg
		if entry.Media.Thumbnail.URL != "" {
			itunesImage = &rssfeed.ItunesImg{URL: entry.Media.Thumbnail.URL}
		}
	...
	// podcast-level author: prefer the configured feed author (an aggregated
	// personal feed has no single channel), fall back to the newest entry's
	// channel only when unset — otherwise the whole show would be mislabeled
	// with whoever happens to be on top (e.g. "Lex Fridman").
	author := fi.Author
	link := entries[0].Author.URI
	if author == "" {
		author = entries[0].Author.Name
	} else {
		link = "" // configured feed isn't a single YouTube channel; no show link to a foreign page
	}
	...
	rss := rssfeed.Rss2{
		...
		ItunesAuthor:   author,
		ItunesExplicit: "no",
		ItunesOwner:    &rssfeed.ItunesOwner{Name: author},
	}

	// set image from config or from channel as rss thumbnail
	image := fi.Image
	if image == "" {
		image = entries[0].Media.Thumbnail.URL
	}
	if image != "" {
